...
function [lattice,ref]=fLattice_setup(geo,state,latticetype)
  ...
  [lattice,ref]=geosetup15(geo);       % Aircraft lattice done
  ...
  [n,m]=find(geo.flapped'); % ' stands for transposition
  ...
  if isempty(m) % Does any flaps have rudder deflections?         
    ...
    for k=1:noof_flaps % Loop all flaps & set according to vector
  ...
  if latticetype==1  % standard VLM, replacing slings with horseshoes.
  
    if dim2==8

      temporary=lattice.VORTEX(:,[1 4 5 8],:); % changing to std VLM
    else
...
loopsperwing=geo.nelem;  % Number of semispanwise partitions for this wing
...
% Here it is reconstructing each wing partition geometry
...
for s=1:noofwings
   for t=1:noofloops(s) %setuploop
      ...
      S(s,t)=geo.b(s,t)*CHORDS(s,t)*((1+geo.T(s,t)))/2;  % trapezoid partition area
...
% setup some reference quantities 

ref.b_ref=config('b_ref');  % clearing ref.b_ref value

if isempty(ref.b_ref)

   B=sum(geo.b,2); % column vector containing the sum of each row: Span of each wing

   ref.b_ref=B(1);	                    % reference span = half-span of first wing
   ref.b_ref=ref.b_ref*(geo.symetric(1)+1); % reference span =      span of first wing
   
end

ref.S_ref=config('S_ref');  % clearing ref.S_ref value
 
if isempty(ref.S_ref)
 
   S_r=sum(S,2);   % column vector containing the sum of each row: Span of each wing
...
ref.C_mac=config('C_mac');   % clearing ref.C_mac value
...
ref.mac_pos=config('mac_pos');  % clearing ref.mac_pos value
...
% Output: collocation points coord, vorticies &  Normals %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  neqns=(nx+fnx)*ny; % panel number (+flap) chord/span-wise
  
  dx=(c*(1-fc)/nx);  % panel chord-wise length (non flap)
  
  if flapped==1
     fdx=(c*fc/fnx); % panel chord-wise length (for flap)
  else
     fdx=0;
  end   
  ...
  dr=[a1' a2']; % panel chord-wise length (+flap)
  ...
  % swirl angle - rotation about axis through quarter-chords (already rotated)
  ...
  % dihedral & swept angle - rotation around x & z axes
