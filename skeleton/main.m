...
geo.flapped=[0];         % flapped partition (wing) boolean bit (2d arr)
geo.nwing=1;             % number of wings (scalar)
geo.nelem=1;             % number of partitions on each wing (1d arr)
geo.CG=[0 0 0];          % System center of gravity (center of rotations)
geo.ref_point=[0 0 0];   % System reference point
geo.symetric=1;		 % Wing symmetry boolean bit (2d array)
geo.startx=0;		 % Partition starting coordinate (2d arr)
geo.starty=0;		 %  ---"----
geo.startz=0;		 %  ---"----
geo.c=1;		 % Root chord (2d array)
geo.foil(:,:,1)={'0'};	 % Partition airfoils (3d arr)	
geo.nx=1;		 % number of panels on chord (2d arr)
geo.TW(:,:,1)=0;	 % partition twist (3d array) 2 outboard
geo.dihed=0;		 % Dihedral (2d arr)	
geo.ny=1;		 % number of panels in span (2d arr)
geo.b=1;		 % span(distance root->tip chord) (2d arr)
geo.T=1;		 % Taper ratio (2d arr)
geo.foil(:,:,2)={'0'};	 % 1 inboard, 2 outboard
geo.SW=0;		 % Sweep (2d arr)
geo.TW(:,:,2)=0;         % partition twist (3d arr) 2 outboard
geo.fc=0;		 % flap chord in percent of wingchord (2d arr)
geo.fnx=0;		 % number of panels on flap chords (2d arr)
geo.fsym=0;		 % flap deflection symmetry boolean bit  (2d arr)
...
state.alphadot=0;       % Angle of attack time derivative  % NOT WRONG ?
state.bethadot=0;       % Angle of sidesliptime derivative % NOT WRONG ?
...
% generate lattice   
 ...
 latticetype=1;   % 1 Fixed wake, standard VLM method
